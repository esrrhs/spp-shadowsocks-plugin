...
func parseEnv() (opts Args, err error) {
	...
	if len(ss_remote_host) == 0 {
		return nil, errors.New("no SS_REMOTE_HOST")
	}
	if len(ss_remote_port) == 0 {
		return nil, errors.New("no SS_REMOTE_PORT")
	}
	if len(ss_local_host) == 0 {
		return nil, errors.New("no SS_LOCAL_HOST")
	}
	if len(ss_local_port) == 0 {
		return nil, errors.New("no SS_LOCAL_PORT")
	}
